...
void STShape::updateNeighboringVerticesMapForFace(const Face& face)
{
    for (size_t ii = 0; ii < 3; ++ii) {
        Index vertexIndex = face.GetIndex(ii);

        for (size_t jj = 0; jj < 3; ++jj) {
            Index neighborIndex = face.GetIndex(jj);

            if (vertexIndex != neighborIndex) {
                mNeighborsMap[vertexIndex].insert(neighborIndex);
            }
        }
    }
}

void STShape::rebuildNeighboringVerticesMap()
{
    mNeighborsMap.clear();
    ...
    for (size_t ii = 0; ii < numFaces; ++ii) {
        ...
        updateNeighboringVerticesMapForFace(face);
    }
}
...
STShape::VertexIndexSet STShape::GetNeighboringVertices(size_t idx)
{
    return mNeighborsMap[idx];
}
