...
__global__
void reduce_minmax_kernel(const float* const d_in,
                          float *d_out,
    ...
    if(id == 0) {
        d_out[blockIdx.x] = shared[id];
    }
}

float reduce_minmax(const float* const d_logLuminance,
                   const size_t numRows,
                   const size_t numCols,
                   int flag) {
    dim3 block_dim(1024);
    ...
    size_t s = size;
    while(1) {
        int grid_x = s / thread_dim.x + 1;
        dim3 grid_dim(grid_x);
        int share_size = 1024 * sizeof(float);

        checkCudaErrors(cudaMalloc(&d_out, sizeof(float) * grid_x));
        reduce_minmax_kernel<<<grid_dim, block_dim, share_size>>>(d_in,
                                                                  d_out,
                                                                  numRows, numCols, flag);
        // free d_in
